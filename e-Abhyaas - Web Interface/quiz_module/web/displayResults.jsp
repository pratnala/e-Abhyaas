     ...
     <%
        ...
        try {
            ...
            while ((strLine = br.readLine()) != null) {
               ...
               File directory = new File("uploads"); /*read folder which contains all result CSV files*/
   String[] readfile; /* array to store names of all files in the above mentioned folder*/
File[] myarray; /* array to store  path of all files in the above mentioned folder*/
String[] testName;
String[] final_testName;
myarray=directory.listFiles();

 int num=0; /*simple counter to check conditions*/
 String selected=request.getParameter("sel"); /* variable 'selected' stores the name of the test selected by the teacher for which he/she wants to view result*/
for (int j = 0; j < myarray.length; j++)
{
    ...
    Connection con=DriverManager.getConnection("jdbc:mysql://localhost:3306/quiz_moduledatabase","root","123");
       ...
       File path=myarray[j]; /*store the path of the file to be read in the variable 'path'*/
...
/*the result CSV file contains values seperated with a semi-colon and each row ending with a comma*/
  ...
  readMarks=readfile[i].split("_");
   String qry="select StudentName from student where MacID='"+readMarks[0]+"'"; /*Each row in CSV contains first entry as the Mac ID of the student tablet whose result follows*/
      ...
      if(rs.next())
        ...
        out.println("<tr>");
  out.println("<td>"+readMarks[0]+"</td>"); /*display Mac ID*/
  out.println("<td>"+stuNm+"</td>"); /*display corresponding student name*/
  
  if(flag==1)
        {

        String studentAnswer=readMarks[1]; /* reading the second entry of each row of CSV file which contains answers given by the student,  answer for each question seperated by ';'*/
		...
		/*Iterate through the answers to evaluate student's marks*/
		 ...
		 {
              marks += Integer.parseInt(answer[questionNumber][0]); /*increment  variable 'marks' denoting total marks by the number of  positive marks for the concerned question */
               noOfRight++; /*counter to count the number of right questions*/


            }
            else if (!ans[questionNumber].equals("-1")) /*if answer given by the student is wrong*/
			{
               marks -= Integer.parseInt(answer[questionNumber][1]); /*deccrement  variable 'marks' denoting total marks by the number of  negative marks for the concerned question */
                noOfWrong++; /*counter to count the number of  questions answered incorrectly by the student*/
            }
            }
